<?php          
    ...
    /* converted structs.h */
    $dust_record_dust_pointer, $planets_record_planet_pointer, $gen_gen_pointer = [];
    ...
    abstract class planet_type { 
        ...
        const tWater = 5; 
        const tIce = 6; 
        const tSubGasGiant = 7; 
        const tGasGiant = 8; 
        const tSubSubGasGiant = 9; 
        const tAsteroids = 10; 
        const t1Face = 11; 
        // etc. 
    } 
    ...
    abstract class planets_record {
        $planet_no = 0;
        $a;					                        /* semi-major axis of solar orbit (in AU)*/
        $e;					                        /* eccentricity of solar orbit		 */
        $axial_tilt;			                    /* units of degrees					 */
        $mass;				                        /* mass (in solar masses)			 */
        $gas_giant;			                        /* TRUE if the planet is a gas giant */
        $dust_mass;			                        /* mass, ignoring gas				 */
        $gas_mass;			                        /* mass, ignoring dust				 */
                                                /*   ZEROES start here               */
        $moon_a;				                    /* semi-major axis of lunar orbit (in AU)*/
        $moon_e;				                    /* eccentricity of lunar orbit		 */
        $core_radius;		                        /* radius of the rocky core (in km)	 */
        $radius;				                    /* equatorial radius (in km)		 */
        $orbit_zone;			                    /* the 'zone' of the planet			 */
        $density;			                        /* density (in g/cc)				 */
        $orb_period;			                    /* length of the local year (days)	 */
        $day;				                        /* length of the local day (hours)	 */
        $resonant_period;	                        /* TRUE if in resonant rotation		 */
        $esc_velocity;		                        /* units of cm/sec					 */
        $surf_accel;			                    /* units of cm/sec2					 */
        $surf_grav;			                        /* units of Earth gravities			 */
        $rms_velocity;		                        /* units of cm/sec					 */
        $molec_weight;		                        /* smallest molecular weight retained*/
        $volatile_gas_inventory;
        $surf_pressure;		                        /* units of millibars (mb)			 */
        $greenhouse_effect;	                        /* runaway greenhouse effect?		 */
        $boil_point;			                    /* the boiling point of water (Kelvin)*/
        $albedo;				                    /* albedo of the planet				 */
        $exospheric_temp;	                        /* units of degrees Kelvin			 */
        $estimated_temp;                            /* quick non-iterative estimate (K)  */
        $estimated_terr_temp;                       /* for terrestrial moons and the like*/
    ...
    abstract class planets_record {
        $planet_no = 0;
        $a = 0.00;					                /* semi-major axis of solar orbit (in AU)*/
        $e = 0.00;					                /* eccentricity of solar orbit		 */
        $axial_tilt = 0.00;			                /* units of degrees					 */
        $mass = 0.00;				                /* mass (in solar masses)			 */
        $gas_giant = 0;			                    /* TRUE if the planet is a gas giant */
        $dust_mass = 0.00;			                /* mass, ignoring gas				 */
        $gas_mass= 0.00;			                /* mass, ignoring dust				 */
                                                    
                                                    /*   ZEROES start here               */
        $moon_a = 0.00;	    			            /* semi-major axis of lunar orbit (in AU)*/
        $moon_e = 0.00;		    		            /* eccentricity of lunar orbit		 */
        $core_radius = 0.00;	    	            /* radius of the rocky core (in km)	 */
        $radius = 0.00; 				            /* equatorial radius (in km)		 */
        $orbit_zone = 0;    			            /* the 'zone' of the planet			 */
        $density = 0.00;	    		            /* density (in g/cc)				 */
        $orb_period = 0.00;			                /* length of the local year (days)	 */
        $day  = 0.00;				                /* length of the local day (hours)	 */
        $resonant_period = 0;	                    /* TRUE if in resonant rotation		 */
        $esc_velocity = 0.00;   		            /* units of cm/sec					 */
        $surf_accel  = 0.00;			            /* units of cm/sec2					 */
        $surf_grav = 0.00;			                /* units of Earth gravities			 */
        $rms_velocity  = 0.00;	    	            /* units of cm/sec					 */
        $molec_weight  = 0.00;		                /* smallest molecular weight retained*/
        $volatile_gas_inventory = 0.00;
        $surf_pressure = 0.00;		                /* units of millibars (mb)			 */
