...
foreach($objects as $name => $object){
    if (strstr($name,"StarGen.csv")) {
        ...
        foreach($json["planetas"] as $planeta) {
            $output = dirname($name)."/".str_replace([" ",'\''],["-",""],$planeta["nome"]).".db";
            ...
            $TIPO = str_replace([" ",'\''],["-",""],$planeta["type"]);
            $RAIO = (int)$planeta["radius"];
            
            if ($TIPO == "1Face" || $TIPO == "1") $TIPO = "Rock";
            
            $pedido = '';
            
            if ($TIPO == "Jovian" || $TIPO == 'Sub-Jovian') {
                $pedido = 'generatejovian';
            }
            elseif ($TIPO == "Rock" || $TIPO == 'Ice' || $TIPO == 'GasDwarf') {
                $pedido = 'generaterockicedwarf';
            }
            
            elseif ($TIPO == "Terrestrial" || $TIPO == 'Martian' || $TIPO == 'Venusian') {
                $pedido = 'generateterravenus';
            }
            else {
                $pedido = '';
            }
            ...
            if ($pedido != '') {
                $request = new Zmsg();
                $request->body_set('{"R":'.$RAIO.',"TIPO":"'.$TIPO.'","OUT":"'.$output.'"}');
                
                $reply = $session->send($pedido, $request);
                if (!$reply) {
                    echo 'falha em '.'{"R":'.$RAIO.',"TIPO":"'.$TIPO.'","OUT":"'.$output.'"}'.PHP_EOL;
                }
                else {
                    echo 'resposta'.$reply->unwrap().PHP_EOL;
                }
                unset($request);
                sleep(1);
            }
            else {
                echo "vazio no tipo ".$planeta["type"].PHP_EOL;
            }
            echo PHP_EOL;
        }
    }
